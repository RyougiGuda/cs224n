...
def naiveSoftmaxLossAndGradient(
    ...
    U, o, vc = outsideVectors, outsideWordIdx, centerWordVec
    y_hat = softmax(np.dot(U, vc))
    loss = -np.log(y_hat[o])
    y_diff=y_hat
    y_diff[o] -= 1
    gradCenterVec=np.dot(U.T, y_diff)
    gradOutsideVecs=np.dot(y_diff[:, np.newaxis], vc[:, np.newaxis].T)#.newaxis to add a dimension,array to column vector
...
def negSamplingLossAndGradient(
    ...
    U, o, vc = outsideVectors, outsideWordIdx, centerWordVec
    gradCenterVec, gradOutsideVecs = np.zeros(vc.shape), np.zeros(U.shape)#store gradients
    y_hat = sigmoid(np.dot(U, vc))
    loss = -np.log(y_hat[o])
    y_diff = y_hat
    y_diff[o] -= 1
    gradCenterVec -= U[o] * y_diff[o]
    gradOutsideVecs[o] += (vc * y_diff[o])#with respect to uo
    for k in range(len(indices)):
        if indices[k] != o:
            neg_sam = indices[k]
            neg_sig = sigmoid(-np.dot(U[neg_sam], vc))
            loss -= np.log(neg_sig)
            gradCenterVec -= np.multiply(U[neg_sam], 1 - neg_sig)
            gradOutsideVecs[neg_sam] += np.multiply(vc, 1 - neg_sig)
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    U, V = outsideVectors, centerWordVectors
    c = word2Ind[currentCenterWord]
    vc = V[c]
    for w in outsideWords:
        w_i = word2Ind[w]
        uw = U[w_i]
        loss_w, gradVC_w, gradU_w = word2vecLossAndGradient(vc, w_i, U, dataset)
        loss += loss_w
        gradCenterVecs[c] += gradVC_w
        gradOutsideVectors += gradU_w
    ### END YOUR CODE
