...
def minibatch_parse(sentences, model, batch_size):
    ...
    unfinished_parses = partial_parses[:]
    unfinished_count=len(unfinished_parses)
    while (len(unfinished_parses)>0):
        unfinished_count = len(unfinished_parses)
        if(unfinished_count<batch_size):
            batch_size=unfinished_count
        ...
        for i in range(batch_size):
            ...
            if (len(minibatch[i].stack) == 1 and len(minibatch[i].buffer) == 0):
                pop_lst.append(i)
        for j in range(len(pop_lst)):
            unfinished_parses.pop(pop_lst[j]-j)
    for k in range(len(sentences)):
        dependencies[k]=partial_parses[k].dependencies
